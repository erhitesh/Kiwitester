...
public class RegistrationScreen extends CommonFeatures {
	...
	public String fillFormAndSend(String username) throws Exception {
		...
		try {
			...
			email = enterEmail();
			selectBirthdate("2000", "Jul", "27");
			// selectGender("Male");
			enterPassword();
			enterLocation("Gurgaon");
			selectSignupRelatedCheckbox();
	...
	public String enterUserFName() {
		String fname = "";
		try {
			element = AndroidAppLocators.getInstance().userFName(driver);
			fname = "QA";
			Keywords.typeText(element, fname);

			if (DriverSession.getStepResult())
				DriverSession.getLastExecutionReportingInstance().teststepreporting(
						"User First Name successfully entered", "PASS", "User First Name should be entered");
			else
				DriverSession.getLastExecutionReportingInstance().teststepreporting(
						"User First Name not successfully entered", "FAIL", "User First Name should be entered");
		} catch (Exception e) {
			DriverSession.getLastExecutionReportingInstance().teststepreporting(
					"Exception Occured...while entering User First Name", "FAIL", "User First Name should be entered");
		}
		return fname;
	}

	public String enterUserLName() {
		String lname = "";
		try {
			element = AndroidAppLocators.getInstance().userLName(driver);
			lname = "Tester";
			Keywords.typeText(element, lname);

			if (DriverSession.getStepResult())
				DriverSession.getLastExecutionReportingInstance().teststepreporting(
						"User Last Name successfully entered", "PASS", "User Last Name should be entered");
			else
				DriverSession.getLastExecutionReportingInstance().teststepreporting(
						"User Last Name not successfully entered", "FAIL", "User Last Name should be entered");
		} catch (Exception e) {
			DriverSession.getLastExecutionReportingInstance().teststepreporting(
					"Exception Occured...while entering User Last Name", "FAIL", "User Last Name should be entered");
		}
		return lname;
	}
	...
	public void selectBirthdate(String year, String month, String day) throws Exception {
		try {
			...
			// Keywords.click(element);
			String strList[] = { month, day, year };
			// Keywords.scrollKeys(driver, strList);

			// element = AndroidAppLocators.getInstance().getYearPicker(driver);
			// Keywords.typeText(element, year);
			// element = AndroidAppLocators.getInstance().getDay(driver, day);
			// Keywords.typeText(element, day);
			// element = AndroidAppLocators.getInstance().getMonthPicker(driver);
			// Keywords.typeText(element, month);
	...
	public void sendMeEmailUpdateCheckbox() {
		try {
			//Keywords.a_Driver.get().scrollToExact("Get Started");
			element = AndroidAppLocators.getInstance().sendMeMailUpdateCheckbox(driver);
			Keywords.click(element);

			if (DriverSession.getStepResult())
				DriverSession.getLastExecutionReportingInstance().teststepreporting(
						"Send Me Email Updated checkbox successfully checked", "PASS",
						"Send Me Email Updated checkbox should be checked");
			else
				DriverSession.getLastExecutionReportingInstance().teststepreporting(
						"Send Me Email Updated checkbox not successfully checked", "FAIL",
						"Send Me Email Updated checkbox should be checked");
		} catch (Exception e) {
			DriverSession.getLastExecutionReportingInstance().teststepreporting(
					"Exception Occured...while check the Send Me Email Updated checkbox", "FAIL",
					"Send Me Email Updated checkbox should be checked");
		}
	}
	...
	public void checkAuthorizeCheckbox() {
		try {
			element = AndroidAppLocators.getInstance().authorizeCheckbox(driver);
			Keywords.click(element);

			if (DriverSession.getStepResult())
				DriverSession.getLastExecutionReportingInstance().teststepreporting(
						"Authorize checkbox successfully checked", "PASS", "Authorize should be checked");
			else
				DriverSession.getLastExecutionReportingInstance().teststepreporting(
						"Authorize checkbox not successfully checked", "FAIL", "Authorize checkbox should be checked");
		} catch (Exception e) {
			DriverSession.getLastExecutionReportingInstance().teststepreporting(
					"Exception Occured...while clicking the Authorize checkbox", "FAIL",
					"Authorize checkbox should be checked");
		}
	}
	
	public void selectSignupRelatedCheckbox() {
		try {
			sendMeEmailUpdateCheckbox();
	...
	public boolean allowToAccessDeviceLocation() {
		boolean accessDeviceLocation = false;
		try {
			element = AndroidAppLocators.getInstance().accessDeviceLocation(driver);
			accessDeviceLocation = Keywords.isElementPresent(element);
		} catch (Exception e) {
		}

		return accessDeviceLocation;
	}

	public void handleAlerts() {
		try {
			if (allowToAccessDeviceLocation()) {
				element = AndroidAppLocators.getInstance().accessDeviceLocation(driver);
				Keywords.click(element);
			}else {}
	...
	public void verifyErrorMessageForRequiredFields() {
		try {
			...
			DriverSession.setStepResult(element.getText().trim().equals("Please enter all required fields."));

			if (DriverSession.getStepResult())
				DriverSession.getLastExecutionReportingInstance().teststepreporting("Alert Message successfully opened",
						"PASS", "Alert Message indicating Please enter all required fields should open");
			else
				DriverSession.getLastExecutionReportingInstance().teststepreporting("Alert Message successfully opened",
						"FAIL", "Alert Message indicating invalid email address should open");
		} catch (Exception e) {
			DriverSession.getLastExecutionReportingInstance().teststepreporting("Alert Message successfully opened",
	...
	public void isGetStartedButtonPresent() {
		try {
			element = AndroidAppLocators.getInstance().getStartedButton(driver);
			DriverSession.getLastExecutionReportingInstance().teststepreporting("Get Started button is present", "PASS",
					"Verifying Get Started button is present");
		} catch (Exception e) {
			DriverSession.getLastExecutionReportingInstance().teststepreporting("Get Started button is not present",
					"FAIL", "Verifying Get Started button is present");
			throw e;
		}
	}
	...
	public void isFacebookButtonPresent() {
		try {
			...
			DriverSession.getLastExecutionReportingInstance().teststepreporting("Facebook button is present", "PASS",
					"Verifying Facebook button is present");
		} catch (Exception e) {
			DriverSession.getLastExecutionReportingInstance().teststepreporting("Facebook button is not present",
					"FAIL", "Verifying Facebook button is present");
			throw e;
		}
	}
	...
	public void enterReferralUserName(String referralUserName) throws Exception {
		try {
			...
			Keywords.click(element);
			DriverSession.getLastExecutionReportingInstance().teststepreporting(
					"Successfully entered the username who referred the current user", "PASS",
					"Entering username who referred the current user");
		} catch (Exception e) {
			DriverSession.getLastExecutionReportingInstance().teststepreporting(
					"Could not successfully enter the username who referred the current user", "FAIL",
					"Entering username who referred the current user");
			throw e;
		}
	}
